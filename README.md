...
状态：**M0–M6 全部里程碑完成**（M5 完善交付 + 最终验收见 `TASKS.md` §4.7；epoll/TLS/POSIX 运行时复核待 Linux 目标环境）。
...
- **M3/M4 协议服务与 CLI**：`forgerelayd` 服务端（单 I/O 线程 Poller + 工作线程池、26 种消息分发）、`frctl` 客户端（全部 §10 命令）、bearer 令牌认证 + 角色矩阵 + 速率限制 + 审计、TOML 配置、JSON 行日志。- **M5 交付完善**：TLS(OpenSSL) 后端（Linux 门控）、并发/错误场景/资源限制测试、systemd 单元与部署示例、OPERATIONS/SECURITY 手册、MIT LICENSE。
